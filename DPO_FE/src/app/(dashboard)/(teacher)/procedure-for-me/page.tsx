...
export default function procedureforme() {
  ...
  function _getCaseProgress(idCase: number): any {
    let   data = getCaseProgress(idCase)
            ...
            {cases?.map((row, index) => (
              ...
              >
                ...
                <TableCell align="left" onClick={e => handlePushToDetail(row)}>
                  <div>
                    <h3 className="capitalize text-orange-400 text-xl" >{row.procedure.name}</h3>
                    <div>
                     <SimpleStepper idCase={row.case.id} />
                    </div>
                  </div>
                </TableCell>
